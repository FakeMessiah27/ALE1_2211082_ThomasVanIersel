...
namespace ALE1_2211082_ThomasVanIersel
{
    ...
    static class QuineMcCluskeyAlgorithm
    {
        public static List<List<LogicState>> MinimizeTruthTable(TruthTable input)
        {
            ...
            {
                List<List<LogicState>> falseRows = new List<List<LogicState>>();
                List<List<LogicState>> trueRows = getRowsWithTrueOutput(input, ref falseRows);
                ...
                List<List<LogicState>> trueLines = PetricksMethod.RemoveNonEssentialPrimeImplicants(trueRows);
                falseRows.AddRange(trueLines);
        ...
        private static List<List<LogicState>> getRowsWithTrueOutput(TruthTable input, ref List<List<LogicState>> falseRows)
        {
            ...
            for (int i = 0; i < input.OutputStates.Count; i++)
            {
                ...
                else if (input.OutputStates[i] == LogicState.False)
                {
                    falseRows.Add(input.InputStates[i].Clone());
                }
            }
